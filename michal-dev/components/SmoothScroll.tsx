...
// Footer Social Icons Spread Effect
export function useFooterSocialIconsSpread(
  footerRef: React.RefObject<HTMLElement>,
  iconsContainerRef: React.RefObject<HTMLElement>
) {
  const { lenis, velocity } = useLenis();
  const quickToRef = useRef<gsap.QuickToFunc | null>(null);
  const [isInViewport, setIsInViewport] = useState(false);
  ...
  useEffect(() => {
    if (prefersReducedMotion || !lenis || !isInViewport || !iconsContainerRef.current) return;

    if (!quickToRef.current) {
      quickToRef.current = gsap.quickTo(iconsContainerRef.current, 'gap', { duration: 0.5, ease: 'power2.out' });
    }

    // Base gap is 8px (gap-2), spread by up to 6px based on velocity
    const baseGap = 8;
    const spread = Math.min(6, Math.abs(velocity) * 1.5);
    const gap = baseGap + spread;

    quickToRef.current(gap);
  }, [velocity, lenis, isInViewport, iconsContainerRef, prefersReducedMotion]);
}
