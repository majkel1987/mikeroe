...
export default function Navbar() {
  ...
  const LangPill = ({ mobile = false }: { mobile?: boolean }) => (
    <button
      ...
      className={`
        ...
        focus-visible:ring-2 focus-visible:ring-sunlit focus-visible:ring-offset-2
        ${mobile
          ? 'bg-summit/80 border-white/10 h-10 w-[72px]'
          : 'bg-summit/60 border-white/8 h-8 w-[62px] hover:border-sunlit/40'
        }
      `}
    >
      {/* Sliding highlight */}
      <span
        className={`
          ...
          bg-ember/90 shadow-[0_0_12px_rgba(226,149,75,0.4)]
  ...
  return (
    <>
      {/* Scroll Progress Bar */}
      <motion.div
        className="fixed top-0 left-0 right-0 h-[2px] bg-ember z-[60] origin-left"
        style={{ scaleX }}
      />
      ...
      <motion.header
        ...
        className={`fixed top-0 left-0 right-0 z-50 transition-colors duration-300 ${
          isScrolled
            ? 'bg-summit/85 backdrop-blur-[16px] border-b border-border/50'
      ...
      >
        <div className="max-w-[1440px] mx-auto w-full px-6 md:px-12 h-16 flex items-center justify-between">
          ...
          <Link href="/" className="flex items-center" onClick={() => { if (lenis) { lenis.scrollTo(0); } else { window.scrollTo({ top: 0, behavior: 'smooth' }); } }}>
            <span className="font-display font-bold text-lg text-snowcap">mikeroe</span>
            <span className="font-display font-bold text-lg text-ember">.pl</span>
          </Link>
            ...
            {navLinks.map((link) => {
              ...
              return (
                <a
                  ...
                  className="relative flex flex-col items-center group cursor-pointer min-h-11 justify-center px-2 rounded-lg focus-visible:ring-2 focus-visible:ring-sunlit focus-visible:ring-offset-2 min-w-[110px] text-center"
                >
                  <span className={`font-sans text-sm transition-colors duration-200 ${
                    isActive ? 'text-snowcap' : 'text-muted group-hover:text-snowcap'
                  }`}>
                    {displayName}
                  </span>
                  ...
                  <div className={`mt-1.5 w-1 h-1 rounded-full transition-all duration-300 ${
                    isActive ? 'bg-sunlit opacity-100 scale-100' : 'bg-sunlit opacity-0 scale-0 group-hover:opacity-50 group-hover:scale-100'
                  }`} />
                </a>
              );
            })}
          ...
          <div className="hidden md:flex items-center gap-3">
            ...
            <motion.a
              ...
              className="group px-5 py-2 min-h-11 min-w-[140px] rounded-xl border border-dusk/50 flex items-center justify-center transition-all bg-transparent hover:bg-dusk focus-visible:ring-2 focus-visible:ring-dusk focus-visible:ring-offset-2"
              whileHover={{ scale: 1.02 }}
              whileTap={{ scale: 0.98 }}
            >
              <span className="font-sans font-medium text-sm text-dusk group-hover:text-bg transition-colors">
          ...
          {/* Mobile Menu Toggle */}
          <button
            className="md:hidden flex flex-col justify-center items-center w-11 h-11 relative z-50 text-snowcap rounded-xl focus-visible:ring-2 focus-visible:ring-sunlit focus-visible:ring-offset-2"
          ...
          >
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div className={`w-1.5 h-1.5 rounded-full transition-all duration-300 ${
                        isActive ? 'bg-sunlit opacity-100' : 'bg-sunlit opacity-0 group-hover:opacity-50'
                      }`} />
            ...
            <div className="mt-8 flex flex-col gap-4">
              ...
              <button
                ...
                className="w-full py-4 rounded-xl border border-dusk bg-dusk/10 flex items-center justify-center text-dusk font-sans font-medium hover:bg-dusk hover:text-bg transition-colors"
              >
                {lang === 'en' ? 'Hire me' : 'Zatrudnij mnie'}
              </button>
            </div>
